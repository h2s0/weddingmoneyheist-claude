...
파일을 만들고 등록해야 하며, `AGENTS.md`의 아키텍처 규칙을 위반하면 안 된다.
시작 전에 `AGENTS.md`를 이미 읽었다고 가정한다. 모호하면 멈추고 묻는다.
...
순서대로 진행한다. 각 단계는 작은 단위로, `AGENTS.md`의 commit 규칙에 맞춰 커밋한다.
...
`AGENTS.md`의 Conventional Commit 제목과 Lore trailer 규칙을 따른다. 예:
