...
## 1순위 — 항상 자동 로드되는 표준 파일 (Claude Code가 스스로 읽음)

1. 현재 파일 (`CLAUDE.md`) — 프로젝트 규칙
2. `~/.claude/CLAUDE.md` — 개인 전역 설정(있는 경우)
3. `.claude/rules/*.md` — 경로별/주제별 세부 규칙(있는 경우)
4. `.claude/skills/*/SKILL.md` — 관련 작업일 때만 자동으로 펼쳐짐(예: 새 위젯 추가 시 `add-widget` 스킬)

## 2순위 — 이 프로젝트의 커스텀 문서 (있을 때만, 표준 파일 아님)

아래는 Claude Code가 자동 인식하지 않는 직접 작성 문서다. 존재할 경우에만 이 순서로 읽는다. 없으면 건너뛴다.
...
> 주의: 위 1순위는 Claude Code가 표준으로 로드하는 파일/폴더이고, 2순위는 이 프로젝트에서만 쓰는 커스텀 문서다. 커스텀 문서가 아직 없다면 만들 필요는 없고, CLAUDE.md 규칙만으로 진행한다.
